...
class SecretCreateView(APIView):
    def post(self, request):
        ser = SecretCreateSerializer(data=request.data)
...
class SecretRevealView(APIView):
    def post(self, request, secret_id):
        ser = SecretRevealSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        ...
        with transaction.atomic():
            ...
            try:
                plaintext = decrypt_secret(obj.ciphertext, password, obj.salt)
            ...
            if obj.remaining_reads is not None and obj.remaining_reads <= 0:
                obj.delete()
            else:
